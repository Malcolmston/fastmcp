...
// Upstream RFC 6570 conformance vectors, transcribed verbatim from the official
// URI Template test suite:
//
//	https://raw.githubusercontent.com/uri-templates/uritemplate-test/master/spec-examples.json
//	https://raw.githubusercontent.com/uri-templates/uritemplate-test/master/extended-tests.json
//
// (repo github.com/uri-templates/uritemplate-test, branch "master"). The JSON is
// embedded unmodified below and decoded at test time, so the templates,
// variable sets and expected expansions are the upstream project's real values,
// not reconstructions. Each testcase is [template, expected] where expected is
// either a single string or, when several orderings are acceptable (associative
// values), a list of acceptable strings. These vectors exercise Expand across
// levels 1-4: simple, +, #, ., /, ;, ?, & operators plus the explode (*) and
// prefix (:n) modifiers, reserved/pct-encoding rules and literal encoding.
...
// parityGroup is one named section of an upstream suite file.
type parityGroup struct {
	Variables map[string]any      `json:"variables"`
	Testcases [][]json.RawMessage `json:"testcases"`
}

// runParitySuite decodes an embedded upstream suite and asserts Expand against
// every testcase, returning (synced, passing).
func runParitySuite(t *testing.T, suite string) (int, int) {
	t.Helper()
	var groups map[string]parityGroup
	if err := json.Unmarshal([]byte(suite), &groups); err != nil {
	...
	for name, g := range groups {
		for _, tc := range g.Testcases {
			...
			var tmpl string
			if err := json.Unmarshal(tc[0], &tmpl); err != nil {
				t.Errorf("%s: bad template entry: %v", name, err)
				continue
			}
			// expected is a string or a list of acceptable strings; a false
			// value marks a negative (invalid-template) case, which this
			// expansion suite does not contain.
			var accept []string
			var single string
			if err := json.Unmarshal(tc[1], &single); err == nil {
				accept = []string{single}
			} else if err := json.Unmarshal(tc[1], &accept); err != nil {
				// non-string, non-array (e.g. false): skip, not an expansion case.
				continue
			}
			synced++
			got, err := Expand(tmpl, g.Variables)
			if err != nil {
				t.Errorf("%s: Expand(%q) error: %v", name, tmpl, err)
				continue
			}
			ok := false
			...
			if ok {
				...
				t.Errorf("%s: Expand(%q) = %q, want one of %q", name, tmpl, got, accept)
			}
		}
	}
	return synced, passing
}

// TestParitySpecExamples runs the upstream spec-examples.json vectors (levels 1-4).
func TestParitySpecExamples(t *testing.T) {
	synced, passing := runParitySuite(t, paritySpecExamplesJSON)
	t.Logf("spec-examples: %d/%d vectors passing", passing, synced)
}

// TestParityExtendedTests runs the upstream extended-tests.json vectors.
func TestParityExtendedTests(t *testing.T) {
	synced, passing := runParitySuite(t, parityExtendedTestsJSON)
	t.Logf("extended-tests: %d/%d vectors passing", passing, synced)
}

const paritySpecExamplesJSON = `
{
  "Level 1 Examples" :
  {
    "level": 1,
