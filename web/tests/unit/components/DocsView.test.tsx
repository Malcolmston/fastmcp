...
const DOC_INDEX: DocIndex = {
  module: 'github.com/malcolmston/fastmcp',
  packages: [
    {
      importPath: 'github.com/malcolmston/fastmcp',
      name: 'fastmcp',
      synopsis: 'Package fastmcp is a standard-library-only Go framework for MCP servers.',
      doc: 'Package fastmcp is a standard-library-only Go framework for MCP servers.',
      consts: [],
      vars: [],
      types: [
        {
      ...
      funcs: [{ name: 'New', signature: 'func New(name string, opts ...Option) *Server', doc: 'New creates a server.' }],
    },
  ],
};
...
describe('DocsView', () => {
  ...
  it('renders the inline React API reference from the fetched doc.json', async () => {
    ...
    expect(await screen.findByRole('heading', { name: /package fastmcp/ })).toBeInTheDocument();
